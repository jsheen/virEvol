...
multistrainSim_mod1 <- function(interyear=interyear_input, maxyear=maxyear_input) {
  ...
  virstrats <- data.frame(matrix(NA, ncol=10, nrow=(maxyear / interyear)))
  ...
  while (endyear < maxyear) {
    ...
    # Check if all extinct extinction
    all_extinct <- TRUE
    for (col_dex in 3:12) {
      if ((out.df[nrow(out.df), col_dex] + out.df[nrow(out.df), col_dex + 10]) > threshold_extinction) {
        all_extinct <- FALSE
    ...
    if (length(which(out.df[nrow(out.df), 13:22] != 0)) == 10) {
      I_dex_setzero <- which(out.df[nrow(out.df), 13:22] == min(out.df[nrow(out.df), 13:22]))[1]
...
finalMatrix <- foreach(i=1:nsim, .combine=rbind) %dopar% {
  ...
  # write.csv(tempMatrix, paste0("~/virEvol/scratch/", i[1], ".csv")) # for debugging purposes
  tempMatrix
}
stopCluster(cl)
