...
# ------------------------------------------------------------------------------
# Modifed (1) to run 1200 months (2) changed time periods of simulation to be 30 days (3) output name
...
multistrainSim_mod4 <- function(interyear=interyear_input, maxyear=maxyear_input) {
  ...
  time <- seq(0, interyear*30, by = 0.1)
...
stopCluster(cl)
write.csv(finalMatrix, paste0('~/virEvol/code_output/multistrain_res/mod4_interyear', interyear_input, '_maxyear', maxyear_input, '_highvirselect_novax_month.csv'))
rm(list = ls())
