...
multistrainSim_mod1 <- function(interyear=interyear_input, maxyear=maxyear_input) {
  ...
  # Last iteration if not all extinct
  if (!all_extinct) {
    if (endyear != maxyear) {
...
stopCluster(cl)
write.csv(finalMatrix, paste0('~/virEvol/code_output/multistrain_res/mod1_interyear', interyear_input, '_maxyear', maxyear_input, '_highvirselect_month.csv'))
rm(list = ls())
