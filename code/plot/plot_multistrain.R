...
# NO VAX: Model 4: SEIR with vaccination and differential migration and slaughter
finalMatrix <- read.csv('~/virEvol/code_output/multistrain_res/mod4_interyear1_maxyear100_highvirselect_novax.csv')
finalMatrix <- finalMatrix[,2:ncol(finalMatrix)]
forhist <- c()
for (row_dex in seq(101, 101000, 101)) {
  forhist <- c(forhist, finalMatrix[row_dex,][which(!is.na(finalMatrix[row_dex,]))])
...
finalMatrix <- read.csv('~/virEvol/code_output/multistrain_res/mod4_interyear1_maxyear100_novax.csv')
finalMatrix <- finalMatrix[,2:ncol(finalMatrix)]
forhist <- c()
for (row_dex in seq(101, 101000, 101)) {
  forhist <- c(forhist, finalMatrix[row_dex,][which(!is.na(finalMatrix[row_dex,]))])
