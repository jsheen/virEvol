...
finalMatrix <- foreach(i=vir_steps, .combine=cbind) %dopar% {
  ...
  library(foreach)
  library(doParallel)
  max_tot_change <- plot_equil(res_vir=i, invade_vir=i, name=paste0('baseline_mod4_tradeoff1 ', i))
  max_tot_change
}
jpeg('~/virEvol/code_output/testing/equil_plot/baseline_mod4_tradeoff1.jpg')
plot(vir_steps, finalMatrix, type='l', main='baseline_mod4_tradeoff1', ylab='tot change in past 100 days')
dev.off()