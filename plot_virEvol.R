...
l <- list()
l_dex <- 1
for (perc_sold_per_farm in seq(0, 0.99, by=0.33)) {
  for (perc_vax in seq(0, 0.66, by=0.33)) {
    pip <- read.csv(paste0('~/virEvol/res/', perc_sold_per_farm, '_', perc_vax, '.csv'))
    pip <- pip[,c(2:ncol(pip))]
    pip <- data.matrix(pip)
    colnames(pip) <- seq(0.01, 99.01, 5)
    rownames(pip) <- rev(seq(0.01, 99.01, 5))
...
ggsave(filename="~/virEvol/output.tiff", marrangeGrob(grobs = l, nrow=3, ncol=4, top='Percent sold per farm in 3 months -->',
