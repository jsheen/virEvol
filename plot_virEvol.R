...
for (perc_sold_per_farm in seq(0, 0.8, by=0.2)) {
  for (perc_vax in seq(0, 0.8, by=0.2)) {
    if (!(perc_sold_per_farm == 0.2 & perc_vax == 0.4)) {
      pip <- read.csv(paste0('~/virEvol/res/', perc_sold_per_farm, '_', perc_vax, '_singular.csv'))
      ...
      l[[l_dex]] <- ggplot(mtcars, aes(x = wt, y = mpg)) + geom_blank()
      l_dex <- l_dex + 1
    }
  }
}
...
ggsave(filename="~/virEvol/output_sensitivity.tiff", marrangeGrob(grobs = l, nrow=5, ncol=5, top='Percent sold per farm in 3 months -->',
                                                      left='<-- Percent vaccinated in 3 months'),
       width=8, height=8, units='in', dpi=600)
ggsave(filename="~/virEvol/output_singular.tiff", marrangeGrob(grobs = l, nrow=5, ncol=5, top='Percent sold per farm in 3 months -->',
                                                                  left='<-- Percent vaccinated in 3 months'),
       width=8, height=8, units='in', dpi=600)
